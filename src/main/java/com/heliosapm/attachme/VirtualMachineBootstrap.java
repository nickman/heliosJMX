...
public class VirtualMachineBootstrap extends BaseWrappedClass {
	...
	private static final SLogger LOG = SimpleLogger.logger(VirtualMachineBootstrap.class);
	...
	protected static void findAttachAPI(String urlLocation) {			
		...
		try {
			...
			LOG.log("Found AttachAPI in Standard ClassPath [" + clazz.getClassLoader() + "]");
			ClassLoader cl = clazz.getClassLoader();
			if(cl==null) {
				cl = ClassLoader.getSystemClassLoader();
			}
			LOG.log("Attach API ClassLoader:" + cl);
...
//		log("This VM:" + vm.toString());
		...
		for(VirtualMachineDescriptor vmd: ap.listVirtualMachines()) {
			LOG.log("Testing VMD (" + vmd.id() + ") [" + vmd.displayName() + "]   Name:" + vmd.provider().name() + "  Type:" + vmd.provider().type());
			if(id.equals(vmd.id())) {
				VirtualMachine vm = ap.attachVirtualMachine(vmd);
				LOG.log("This VM:" + vm.toString());	
				Properties agentProps = vm.getAgentProperties();
				for(Map.Entry<Object, Object> p: agentProps.entrySet()) {
					LOG.log("\t\t" + p.getKey() + ":" + p.getValue());
				}
			}
