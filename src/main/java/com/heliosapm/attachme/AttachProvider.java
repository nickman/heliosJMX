...
import com.heliosapm.jmx.util.helpers.XMLHelper;
...
public class AttachProvider extends BaseWrappedClass {
	...
	private static final SLogger LOG = SimpleLogger.logger(AttachProvider.class);
	...
	static void init() {
		...
		try {
			pushCl();			
			LOG.log("Loading Attach Provider with [" + Thread.currentThread().getContextClassLoader() + "]");
